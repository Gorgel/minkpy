...
void main(int nar, char* ar[])
{
  ...
  while(++n<nar){
    if(ar[n][0]=='-'&&ar[n][1]){
      ...
      case'c' : par.cstyle  =1      ; break;
      case'C' : par.complement  =1      ; break;
      case'd' : par.floattyp=0      ; break;
      case'f' : par.floattyp=1      ; break;
  ...
  /* header size */
  if (par.cstyle) headersize=12; else headersize=24;
 ...
 /*   printf("%s"," here 1a \n");
    ...
    printf("size = %d\n",sizeof(in));   */
  if(in) {
    /* read header with data size */

    fread((void*)buffer,sizeof(char),headersize,in);

    f=vector(0,n);
    if (par.floattyp) {

      printf("%s"," reading float\n"); 
	...
	exit(99);
      ...
      printf("%s"," reading double\n"); 
      fd=doublevector(0,n);
      if(n!=fread((void*)fd,sizeof(double),n,in)) {
	fprintf(stderr,"error when reading input!\n"); 
	exit(99);
      }
      for(i=0;i<n;i++) u[2*i]=(float)fd[i],u[2*i+1]=0; 
      free_doublevector(fd,0,n);
    }

    if (par.complement) {
      par.lo=(-1.0)*par.lo;
      par.hi=(-1.0)*par.hi;
      for(i=0;i<n;i++) u[2*i]=(-1.0)*u[2*i];
    }
